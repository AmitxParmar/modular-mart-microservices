# 🚀 Modular Mart: Microservices Task List

This document tracks the tasks for transitioning to a production-grade, event-driven microservices architecture.

## 🏗️ Roadmap & Progress Checklist

### 🏁 Phase 1: Service Extraction
- [x] **Catalog Service Extraction**: Decouple products/categories to `catalog-service` with its own database.
- [x] **Order Service Extraction**: Relocate order processing to `order-service` and drop direct DB schema imports.
- [x] **Payment Service Extraction**: Decouple Stripe checkout and webhook handling to `payment-service`.
- [x] **API Gateway Alignment**: Route and proxy client requests dynamically to the correct downstream microservice.
- [x] **Storefront Integration**: Update the Next.js frontend to handle the decoupled HTTP routes and JWT claims.

---

### 🔄 Phase 2: Asynchronous Saga (Checkout & Stock Reservation)
- [x] **State Machine Extension**: Introduce intermediate order statuses (`PENDING_STOCK`, `STOCK_CONFIRMED`, `STOCK_FAILED`, `PAYMENT_PENDING`).
- [x] **Order Service Consumers**: Register event patterns for `STOCK_RESERVED` and `STOCK_RESERVE_FAILED`.
- [x] **Catalog Service Saga Integration**: Evolve the reservation service to listen to reservation requests and publish reservation status events asynchronously.
- [x] **Compensation Logic (Rollbacks)**: Verify automatic refunding and order cancellation when reservations fail.

---

### 🛡️ Phase 3: Reliability & Resiliency Patterns
- [x] **Idempotent Consumers**: Implement message deduplication using a `processed_messages` schema to prevent double-processing.
- [x] **Transactional Outbox Pattern**: Insert order states and outbox events in a single transaction to guarantee event delivery.
- [x] **Retry Policies & DLQs**: Set up dead-letter exchanges and queues in RabbitMQ to handle transient errors gracefully.
- [x] **Distributed Tracing**: Standardized payloads to carry `correlationId` and integrate with Jaeger.
- [ ] **Circuit Breaker Pattern**: Implement circuit breakers for all synchronous service-to-service communication to prevent cascading failures.
- [ ] **Cache Aside Pattern**: Integrate Redis for catalog and search result caching to reduce database load and improve response times.
- [ ] **Pub/Sub Messaging Topology**: Transition all services to publish to a shared `mart.events` Topic Exchange instead of direct-to-queue routing.
- [ ] **Security**: Implement Service-to-Service mTLS and enhanced API security.

---

### 📢 Phase 4: Operations & Integrations
- [ ] **Redeploy to Render**: Update `render.yaml` with new service configurations and trigger a fresh deployment.
- [x] **Notification Service**: Build a lightweight email/SSE service consuming order/payment success events.
- [ ] **API Gateway Circuit Breakers**: Protect API Gateway routes with circuit breakers to fail-fast under heavy loads.

---
*Last Updated: June 3, 2026*

